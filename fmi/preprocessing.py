...
__all__ = ['mask_and_save_path', 'mask_and_save_df', 'process_dicom', 'image_hist', 'move_files_df', 'dicomsplit',
...
# Cell
def process_dicom(path, outpath, window, sigma, thresh, remove_max, sanity=False, show=False):
    " Convert .dcm images into .png format specifying parameters: window width, center, sigma, thresh"
    ...
    mask = dicom.mask_from_blur(window, sigma=sigma, thresh=thresh, remove_max=remove_max)
    bbs = mask2bbox(mask)
    lo,hi = bbs
    r = data[lo[0]:hi[0],lo[1]:hi[1]]
    ...
    #cropped image
    height2 = len(r); width2 = len(r[0])
    pixels_out2 = []
    for row2 in r:
        pixels_out2.extend(row2)
    assert(len(pixels_out2) == height2 * width2)

    image_out2 = Image.new('L', (width2, height2))
    image_out2.putdata(pixels_out2)

    #save image to outpath
    image_out2.save(f'{outpath}')
    if sanity:
        print(f'hi: {hi} lo: {lo}')
        print(f'original shape: {data.shape}')
        print(f'cropped shape: {image_out2.shape}')
    if show:
        show_images([data, image_out2], titles=['original', ' saved crop'])
...
def move_files_df(df, column_name, file_name=None):
    ...
    try:
        for i in df.index:
            ...
            shutil.copy(img_file, save_path, follow_symlinks=True)
