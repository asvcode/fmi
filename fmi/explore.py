...
__all__ = ['system_info', 'random_', 'get_image_info', 'get_pii', 'instance_sort', 'instance_dcmread', 'instance_show',
           'scaled_px', 'get_dicom_image', 'dicom_convert_3channel', 'show_aspects', 'draw_outline', 'draw_rect',
           'TensorBBox', 'LabeledBBox']
...
# Cell
def draw_rect(ax, b, color='black', text=None, text_size=14, hw=True, rev=False):
    lx,ly,w,h = b
...
class TensorBBox(TensorPoint):
    ...
    def show(self, color, ctx=None, **kwargs):
        x = self.view(-1,4)
        for b in x: draw_rect(ctx, b, hw=False, color=color, **kwargs)
        return ctx
